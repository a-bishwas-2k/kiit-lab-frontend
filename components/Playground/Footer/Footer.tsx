    ...
    <div className=' text-white bg-[#1f1f1f] p-3'>
        { TestCases[currentTestCases].output}
            ...
            {message.status !== 0 && <div className=' p-3 flex gap-2 flex-col'>
                ...
                <span>Message:</span>
                <div className=' text-white bg-[#1f1f1f] p-3'>
                    {message.status==3?<span className='text-green-400'>All Test Cases Passed Successfully</span>:message.status==4?<span className='text-red-400'>Some test cases failed</span>: message.compile_output}
                </div>
            </div>
            }
